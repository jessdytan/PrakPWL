    ...
    <div class="col-10">
        <form action="/admin/store_post" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('excerpt') is-invalid @enderror" id="excerpt" required name="excerpt" >{{ old('excerpt') }}</textarea>
                @error('excerpt')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" required name="content">{{ old('content') }}</textarea>
                @error('content')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
